...
def run_game(
    ...
    white_time = DEFAULT_TIME_SECONDS * 1000
    black_time = DEFAULT_TIME_SECONDS * 1000
    ...
    while not board.is_game_over(claim_draw=True):
        ...
        if board.turn == chess.WHITE:
            bestmove = get_bestmove(moves, white_engine, white_time, black_time)
        else:
            bestmove = get_bestmove(moves, black_engine, white_time, black_time)
        end_time = time.time_ns()
        elapsed_time_ns = end_time - start_time
        elapsed_time_ms = (elapsed_time_ns // 1_000_000) + 1
        if board.turn == chess.WHITE:
            white_time -= elapsed_time_ms
        else:
            black_time -= elapsed_time_ms
        ...
        if white_time < 0:
            ...
            return (GameOutcome.BLACK_WIN_TIME, moves)
        if black_time < 0:
            white_engine.terminate()
            black_engine.terminate()
            return (GameOutcome.WHITE_WIN_TIME, moves)
    white_engine.terminate()
    black_engine.terminate()
...
def get_bestmove(
    ...
    w_time: Optional[int] = None,
    b_time: Optional[int] = None,
) -> str:
    assert engine_process.stdout is not None
    assert engine_process.stdin is not None
    ...
    if w_time is not None and b_time is not None:
        engine_process.stdin.write(f"go wtime {w_time} btime {b_time} \n") 
        engine_process.stdin.flush()
    else:
    ...
    # print(f" Bestmove line = {best_move_line}")
...
def blocking_readline(engine_process: subprocess.Popen, should_print: bool = False):
    ...
    while line is None and counter <= DEFAULT_TRIES:
        ...
        counter += 1
        if line is None:
            time.sleep(SLEEP_START_TIME * (counter**1.5))
    if line is None:
...
def write_game_to_disk(game: GameResult, file: TextIO):
    ...
    pgn_game.headers["White"] = "White Engine" 
    pgn_game.headers["Black"] = "Black Engine" 
